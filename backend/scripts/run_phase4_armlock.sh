#!/usr/bin/env bash
# Phase 4 refinement sweep (2026-08-30): trail_lock_fraction bracket (0.4,
...
SHARD_COUNT=28
NEAR_EXPIRY_DAYS=6
DB_PREFIX="trading_bot_backtest_s4p4_"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
RESULTS_DIR=data/historical/backtest_reports/s4p4_refinement_${STAMP}
STATUS_FILE=~/s4p4_status.log
LOG_DIR=/tmp/s4p4_logs
...
log "=========================================="
log "Phase 4 refinement sweep starting -> $RESULTS_DIR ($n_configs configs)"
log "=========================================="
sweep_start=$(date +%s)
