#!/usr/bin/env bash
# Sweep #3 W7 -- Lorentzian Classification ("Framework 5"), standalone.
# No Postgres, no --db-suffix, no reaper needed: scripts/loren_backtest.py is
# self-contained (pandas/numpy only). Run under ~/an_venv on the e4 VM.
#
#   cd ~/trading-bot/backend
#   setsid bash scripts/run_loren_sweep_3w7.sh </dev/null >/tmp/sweep3w7_nohup.log 2>&1 & disown
#   tail -f /tmp/sweep3w7_status.log
#
# ETA on e4 (SHARDS=12): ~2-4 min total. Then read the summary block at the
# bottom of the status log, pull the results dir, run analyze_walkforward.py.
set -u
