...
STATUS_FILE=~/trading-bot/backtest_status_banknifty.log
LOG_DIR=/tmp/backtest_logs_banknifty
...
for strat in $STRATEGIES; do
  ...
  for i in $(seq 0 $((SHARD_COUNT - 1))); do
    ./.venv/bin/python scripts/run_backtest.py \
      --strategy "$strat" --underlying BANKNIFTY \
      ...
      --db-suffix "${strat}_BANKNIFTY_shard${i}" \
      --out-csv "${RESULTS_DIR}/${strat}_BANKNIFTY_trades_shard${i}.csv" \
  ...
  for mode in $MODES; do
    if ! ./.venv/bin/python scripts/merge_backtest_shards.py \
      --glob "${RESULTS_DIR}/${strat}_BANKNIFTY_trades_shard*_${mode}.csv" \
      --out "${RESULTS_DIR}/${strat}_BANKNIFTY_trades_${mode}.csv" \
  ...
  if ./.venv/bin/python scripts/summarize_exit_modes.py \
    --strategy "$strat" --underlying BANKNIFTY --total-lots 10 \
    --data-dir "$RESULTS_DIR" > "${RESULTS_DIR}/${strat}_BANKNIFTY_summary.txt" 2>&1; then
    log "$strat: summary written -> ${RESULTS_DIR}/${strat}_BANKNIFTY_summary.txt"
  else
    log "*** $strat: summarize_exit_modes.py FAILED -- check ${RESULTS_DIR}/${strat}_BANKNIFTY_summary.txt ***"
  fi
