#!/usr/bin/env bash
# Phase 3 refinement sweep (2026-08-30): trail_lock_fraction bracket (0.4,
...
SHARD_COUNT=28
NEAR_EXPIRY_DAYS=6
DB_PREFIX="trading_bot_backtest_s4p3_"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
RESULTS_DIR=data/historical/backtest_reports/s4p3_refinement_${STAMP}
STATUS_FILE=~/s4p3_status.log
LOG_DIR=/tmp/s4p3_logs
CONFIG_FILE="${1:?usage: run_phase3_refinement.sh <config_list_file>}"
...
log "=========================================="
log "Phase 3 refinement sweep starting -> $RESULTS_DIR ($n_configs configs)"
log "=========================================="
sweep_start=$(date +%s)
...
log "=========================================="
log "S4P3 REFINEMENT SWEEP COMPLETE -> $RESULTS_DIR ($(( ($(date +%s) - sweep_start) / 60 ))min total)"
log "=========================================="
