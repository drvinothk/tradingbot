...
class MockBrokerAdapter(BrokerPort):
    ...
    def get_recent_trades(self, contract_symbol: str) -> list[TradeFill]:
        """Unconditionally `[]` -- deliberately, not a stub to fill in
        later. Reconciliation's auto-repair path (see `BrokerPort.
        get_recent_trades`'s own docstring) exists to recover a real fill
        for a position closed by a human *outside this system*, directly in
        a real broker's app. Nothing outside this codebase can ever touch
        the mock's own position book -- every paper fill is generated by
        this same class's own `place_order` -- so a paper-side local-open/
        broker-flat mismatch is always a different bug (the mock's
        process-memory book losing state across a restart, already fixed
        via `seed_position`/`rebuild_execution_mock_position_book`, see
        CLAUDE.md's 2026-08-27 entry), never "a fill this function could
        find." Returning `[]` correctly leaves that class of mismatch on
        its existing alert-only path rather than inventing a fill.
        """
        return []
