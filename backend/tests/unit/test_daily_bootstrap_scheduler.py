...
def test_does_not_trigger_before_bootstrap_time(monkeypatch, calls):
    monkeypatch.setattr(bootstrapper_module, "now_ist", lambda: _at(8, 59))
...
def test_triggers_on_first_tick_at_or_after_bootstrap_time(monkeypatch, calls):
    monkeypatch.setattr(bootstrapper_module, "now_ist", lambda: _at(9, 0))
    scheduler = DailyBootstrapScheduler()
...
def test_does_not_retrigger_later_the_same_day(monkeypatch, calls):
    ...
    monkeypatch.setattr(bootstrapper_module, "now_ist", lambda: _at(12, 0))
    scheduler.run_once()
...
def test_triggers_again_the_next_day(monkeypatch, calls):
    monkeypatch.setattr(bootstrapper_module, "now_ist", lambda: _at(9, 0, day=18))
    scheduler = DailyBootstrapScheduler()
    scheduler.run_once()

    monkeypatch.setattr(bootstrapper_module, "now_ist", lambda: _at(9, 0, day=19))
    scheduler.run_once()
...
def test_a_restart_at_09_05_catches_up_immediately(monkeypatch, calls):
    ...
    monkeypatch.setattr(bootstrapper_module, "now_ist", lambda: _at(9, 5))
    scheduler = DailyBootstrapScheduler()
...
def test_does_not_mark_the_day_done_when_bootstrap_raises(monkeypatch):
    ...
    assert scheduler._last_bootstrap_date is None
