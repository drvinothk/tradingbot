...
class TestLiquiditySweepReversalStrategy:
    ...
    def test_genuine_breakout_that_closes_beyond_range_does_not_fire(
        ...
        strategy's territory is a false breakout that reverses, so a
        genuine continuation must not fire here."""
        start = datetime(2026, 7, 24, 9, 15, tzinfo=UTC)
        strategy_run = _make_strategy_run(db, strategy_config, trading_session, user, start)
        _seed_chain(db, instrument, option_contract_ce, option_contract_pe)
        _seed_window(db, instrument, start, window_high=22050.0, window_low=21950.0)
    ...
    def test_swept_high_reverses_down_fires_buy_pe(
        self, db: Session, instrument, option_contract_ce, option_contract_pe, trading_session,
        strategy_config, user,
    ):
        start = datetime(2026, 7, 24, 9, 15, tzinfo=UTC)
        ...
        sweep_bar = _seed_bar(
            db, instrument, start + timedelta(minutes=LOOKBACK_BARS),
            open=22030, high=22070, low=22020, close=22040,
        )

        strategy = LiquiditySweepReversalStrategy(
            instrument.id, EXPIRY, lookback_bars=LOOKBACK_BARS
        )
        proposal = strategy.check_setup(db, strategy_run, sweep_bar)
    ...
    def test_swept_low_reverses_up_fires_buy_ce(
        self, db: Session, instrument, option_contract_ce, option_contract_pe, trading_session,
        strategy_config, user,
    ):
        start = datetime(2026, 7, 24, 9, 15, tzinfo=UTC)
    ...
    def test_can_fire_again_after_a_prior_sweep_this_run(
        self, db: Session, instrument, option_contract_ce, option_contract_pe, trading_session,
        strategy_config, user,
    ):
        """Unlike ORB/OI-Volume-Confirmed, no per-direction fired-once guard
        — the rolling window shifts, so a second genuine sweep-and-reversal
        setup later in the same run must still fire."""
        start = datetime(2026, 7, 24, 9, 15, tzinfo=UTC)
        strategy_run = _make_strategy_run(db, strategy_config, trading_session, user, start)
        _seed_chain(db, instrument, option_contract_ce, option_contract_pe)
        _seed_window(db, instrument, start, window_high=22050.0, window_low=21950.0)
        ...
        first = strategy.check_setup(db, strategy_run, sweep_bar)
        second = strategy.check_setup(db, strategy_run, sweep_bar)
